...
mk_kernel := + $(hide) $(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(KBUILD_OUTPUT) ARCH=$(TARGET_ARCH) CROSS_COMPILE=$(abspath $(TARGET_TOOLS_PREFIX)) $(if $(SHOW_COMMANDS),V=1)

KERNEL_CONFIG_FILE := $(if $(wildcard $(TARGET_KERNEL_CONFIG)),$(TARGET_KERNEL_CONFIG),$(TARGET_KERNEL_SOURCE)/$(KERNEL_CONFIG_DIR)/$(TARGET_KERNEL_CONFIG))
...
WL_PATH := $(TARGET_KERNEL_SOURCE)/drivers/net/wireless/wl
